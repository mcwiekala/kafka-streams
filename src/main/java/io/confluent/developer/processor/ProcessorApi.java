...
public class ProcessorApi {
        ...
        public Processor<String, ElectronicOrder, String, Double> get() {
            return new Processor<>() {
                ...
                private void forwardAll(final long timestamp) {
                    ...
                    try(KeyValueIterator<String, Double> iterator = store.all()){
                        while (iterator.hasNext()) {
                            ...
                            Record record = new Record(next.key, next.value, timestamp);
                            context.forward(record);
                            System.out.println("Punctuation forwarded record - key " + record.key() + " value " + record.value());
                        }
                    }
                ...
                public void process(Record<String, ElectronicOrder> record) {
                    ...
                    Double actualPrice = store.get(key);
                    if(actualPrice == null) {
                        actualPrice = 0.0;
                    }
                    double updatedPrice = record.value().getPrice() + actualPrice;
                    store.put(key, updatedPrice);
    ...
    public static void main(String[] args) throws IOException {
        ...
        topology.addSource("source", stringSerde.deserializer(), electronicSerde.deserializer(), inputTopic);
        ...
        topology.addProcessor("processor", new TotalPriceOrderProcessorSupplier(storeName), "source");
        ...
        topology.addSink("sink", outputTopic, stringSerde.serializer(), doubleSerde.serializer(), "processor");
